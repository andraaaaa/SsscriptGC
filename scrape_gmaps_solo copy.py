...
def search_place(page: Page, search_for: str):
    ...
    try:
        ...
        single = page.locator('//div[@class="aoRNLd kn2E5e lvtCsd "]')
        multi_results = page.locator('//a[contains(@href, "https://www.google.com/maps/place")]')
        if page.is_visible('//div[@class="aoRNLd kn2E5e lvtCsd "]'):
            print("ada")
            try:
                ...
                if place.name:
                    ...
                    return "Selector gak ada cuy"
            except: print(f'Tidak menemukan hasil untuk tempat {place}')

        elif multi_results.count() > 0:            
            try:
                ...
                return None
            except: print(f'Gagal mengambil banyak tempat dengan keyword {search_for} : {e}')     
    except Exception as e:
...
def main():
    ...
    with open('SsscriptGC\Book3.csv', 'r') as f:
        ...
        for i, l in enumerate(lists, start=1):
            try:
                ...
                if place is not None:
                    ...
                    d_row.to_csv(csv_output, mode='a', header=first_write, index=False)
                    first_write = False
            except Exception as e:
